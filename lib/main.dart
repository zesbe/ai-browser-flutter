...
import 'dart:ui';
import 'dart:math'; // For min/max logic
...
// --- PROVIDERS (Same core logic, added Print) ---
...
class BrowserProvider extends ChangeNotifier {
  ...
  // --- FEATURES ---
  void panicButton() {
    // Close all incognito tabs and go home
  ...
  void printPage() async {
    await currentTab.controller?.printCurrentPage();
  }

  void swipeTab(bool isLeft) {
    // Swipe Capsule Logic
    if (isLeft) { // Swipe Right -> Prev
  ...
  // ... (Standard Logic: Biometric, Bookmarks, Downloads, Ads, Find, etc.)
  Future<void> checkBiometricLock(BuildContext ctx) async { if (!isBiometricEnabled) { isLocked = false; notifyListeners(); return; } try { if (await auth.authenticate(localizedReason: 'Unlock', options: const AuthenticationOptions(stickyAuth: true))) { isLocked = false; notifyListeners(); } } catch (e) {} }
  ...
  Future<void> importData(BuildContext ctx, String s) async { try { final d = jsonDecode(s); if (d['history'] != null) history = (d['history'] as List).map((e) => HistoryItem.fromJson(e)).toList(); if (d['bookmarks'] != null) bookmarks = (d['bookmarks'] as List).map((e) => BookmarkItem.fromJson(e)).toList(); if (d['speedDials'] != null) speedDials = (d['speedDials'] as List).map((e) => SpeedDialItem.fromJson(e)).toList(); await _saveData(); notifyListeners(); ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text("Restored"))); } catch (e) { ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text("Error"))); } }
...
class AiAgentProvider extends ChangeNotifier {
  ...
  void sendMessage(String text, BrowserProvider b) async {
    ...
    String resp = "OK.";
    if (text.contains("scan")) { resp = "Opening Scanner..."; } 
    else if (text.contains("home")) { b.loadUrl("neon://home"); resp = "Going Home."; }
    else { resp = "I can navigate, scan QR codes, or manage bookmarks."; }
...
class _BrowserHomePageState extends State<BrowserHomePage> with TickerProviderStateMixin {
  ...
  @override void initState() { super.initState(); _menuController = AnimationController(vsync: this, duration: const Duration(milliseconds: 150)); _menuScale = CurvedAnimation(parent: _menuController, curve: Curves.easeOutBack); _pullToRefreshController = PullToRefreshController(settings: PullToRefreshSettings(color: const Color(0xFF00FFC2), backgroundColor: Colors.black), onRefresh: () async { final b = Provider.of<BrowserProvider>(context, listen: false); if (b.currentTab.url != "neon://home") b.reload(); _pullToRefreshController.endRefreshing(); }); 
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(resizeToAvoidBottomInset: false, backgroundColor: Colors.black, body: Stack(children: [
      ...
      if (!browser.showAiSidebar && !browser.isZenMode) AnimatedPositioned(duration: const Duration(milliseconds: 200), bottom: 20, left: 20, right: 20, child: GestureDetector(
        onHorizontalDragEnd: (details) { if (details.primaryVelocity! > 0) browser.swipeTab(true); else if (details.primaryVelocity! < 0) browser.swipeTab(false); },
...
// ... (StartPage, TabGridPage, AiSidebar, etc.)
