...
class BrowserProvider extends ChangeNotifier {
  ...
  Future<void> savePageOffline(BuildContext ctx) async { 
    try { 
      ...
      final a = await currentTab.controller?.saveWebArchive(filePath: path, autoname: false); 
      if (a != null) { addDownload("Archive: $a"); ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text("Saved Offline"))); } 
...
class _BrowserHomePageState extends State<BrowserHomePage> with TickerProviderStateMixin {
  ...
  @override void dispose() { _menuController.dispose(); _visualizerController.dispose(); super.dispose(); }
  ...
  Widget _buildGridMenu(BuildContext context, BrowserProvider b) => GridView.count(shrinkWrap: true, crossAxisCount: 5, mainAxisSpacing: 15, padding: const EdgeInsets.all(16), children: [
    _menuItem(Iconsax.arrow_left_2, "Back", b.goBack), _menuItem(Iconsax.arrow_right_3, "Fwd", b.goForward), _menuItem(Iconsax.refresh, "Reload", b.reload), _menuItem(Iconsax.scan_barcode, "Scan", () { b.toggleMenu(); Navigator.push(context, MaterialPageRoute(builder: (_) => const ScannerPage())); }), _menuItem(Iconsax.game, "Game", () { b.toggleGameMode(); b.toggleMenu(); }), _menuItem(Iconsax.code_circle, "Script", () { b.toggleMenu(); _showScriptManager(context, b); }), _menuItem(Iconsax.volume_high, "Read", () { b.toggleTts(); b.toggleMenu(); }), _menuItem(Iconsax.document_download, "Files", () { b.toggleMenu(); _showDownloads(context, b); }), _menuItem(Iconsax.setting, "Settings", () { b.toggleMenu(); _showSettingsModal(context, b); }), _menuItem(Iconsax.home, "Home", () { b.loadUrl("neon://home"); b.toggleMenu(); }),
  ]);
